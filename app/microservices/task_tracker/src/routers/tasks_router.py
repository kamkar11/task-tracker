...
from common.dependencies.session import SessionAnnotatedDep as SessionDep
...
@api_tasks_router.post("", status_code=status.HTTP_200_OK, response_model=Task)
async def create_task(task: TaskCreate, db: SessionDep):
    return TaskRepository(db).create(task)

@api_tasks_router.get("", status_code=status.HTTP_200_OK, response_model=list[Task])
async def read_tasks(db: SessionDep):
    return TaskRepository(db).get_all()

@api_tasks_router.get("/{task_id}", status_code=status.HTTP_200_OK, response_model=Task)
...
@api_tasks_router.put("/{task_id}", status_code=status.HTTP_200_OK, response_model=Task)
...
@api_tasks_router.delete("/{task_id}", status_code=status.HTTP_200_OK, response_model=Task)
